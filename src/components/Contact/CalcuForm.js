    ...
    <div>
      <select ref={register} name="material">
        ...
        {materialLists.map((list, index) => (
          <option value={list.materialValue} key={index}>{list.materialName}</option>
        ))}
      </select>
      ...
      <select ref={register} name="thicknessValue">
        ...
        {thickness.map((thick, index) => (
          <option value={thick} key={index}>{thick}</option>
        ))}
      </select>

      <input name="length" type="number"  placeholder="Enter length (in mm)" ref={
        register({pattern: { value: /^[0-9]*$/, message: 'numbers only'}})
        } 
      />
      ...
      <input name="width" type="number" placeholder="Enter width (in mm)" ref={
        register({pattern: { value: /^[0-9]*$/, message: 'numbers only'}})
        } 
      />
